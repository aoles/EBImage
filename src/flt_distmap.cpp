...
See flt_distmap.h for license
------------------------------------------------------------------------- */
#include "flt_distmap.h"
...
inline void lz_dist_map(double * data, int& ncol, int& nrow) {
    ...
    try {
        ...
        for (i = 0; i < ncol; i++) {
            inc = 1;
            for (j = 1; j < nrow; j++)
                if (data[getindex(i, j, ncol)] > data[getindex(i, j-1, ncol)] + inc) {
                    data[getindex(i, j, ncol)] = data[getindex(i, j-1, ncol)] + inc;
                    inc += 2;
                }
                else
                   inc = 1;
