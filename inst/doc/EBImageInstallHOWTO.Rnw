...
For \lib{ImageMagick}, you will need to install  \code{libmagick9-dev} with all dependencies using \code{apt-get} or the Synaptic package manager. Alternatively, you can download the latest version of \lib{ImageMagick} from \url{http://www.imagemagick.org} and install it from source as usual using \code{./configure \&\& make \&\& make install}. The latter will install all the required development files as well.
