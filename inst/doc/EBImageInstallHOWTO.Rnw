...
The package was tested to work on Linux (32/64 bit), MacOS, Windows XP (32 bit) and Solaris (64 bit). It should be possible to install and run the package on other versions of UNIX-like and Windows systems in a similar manner. It is advisable to install recommended dependencies as well to enjoy the full functionality of the package. On Windows, all dependencies are required, not recommended. \lib{GTK+} support was not tested on MacOS, but there is no apparent reason for the package to fail to support it.
...
On most Linux distributions, these are available as precompiled binaries \lib{rpm} or \lib{deb} format with a \code{-dev} or \code{-devel} suffix. Hereafter, the package names are for Ubuntu Linux, but other distributions have similar packages with only slight naming differences.

For \lib{ImageMagick}, you will need to install  \code{libmagick9-dev} with all dependencies using \code{apt-get} or Synaptic. Alternatively, you can download the latest version of \lib{ImageMagick} from \url{http://www.imagemagick.org} and install it from source as usual using \code{./configure \&\& make \&\& make install}. The latter will install all the required development files as well.
...
The author did not have any possibility to check if \lib{GTK+}-enabled version works on MacOS due to the lack of hardware when the \lib{GTK+} support was added, but since \code{gtk+2-dev} and \code{gtk+2-shlib} are available at \lib{fink}, it is worth trying. Please remember again, that you will need \code{pkgconfig} (also available from \lib{fink}), in order for the installation script to find your \lib{GTK+} includes.

%--------------------------------------------------
\section{Windows 32bit Installation}
%--------------------------------------------------

The author did not check if the package compiles on Windows 64 bit due to the lack of the corresponding hardware. The Windows package contains a \lib{DLL} that was precompiled on 32 bit Windows XP machine. The tarball is distributed with a \emph{precompiled} Windows \lib{DLL} included in \code{.\\inst\\winlibs\\}. This file is moved to the package's \code{.\\lisb\\} during the Windows build process (\code{R CMD build --binary}) avoiding the compilation. If you want to recompile the source package on Windows, please delete the \code{.\\inst\\winlibs\\} directory from the source tarball.
...
After starting the installation wizard, you can select the default settings and let the installer to register environmental variables for you. Ensure that the path to \lib{ImageMagick} was added to your \code{PATH} variable, e.g. that it contains somthing like \code{c:\\ImageMagick;}.

If you are planning to compile \Rpackage{EBImage} from source, select during the installation that you want development files to be installed as well of. In this case, it is strongly recommended to install \lib{ImageMagick} into a path that does not contain spaces. The recommended, and hard coded into \code{.\\src\\Makevars.win} is \code{c:\\ImageMagick}. If select another location, please be sure to update this file.
...
Please ensure now that the installer added the path to \lib{GTK+} \code{\\bin} directory \emph{before} the path to \code{c:\\Windows\\} and \code{c:\\Windows\\System32}. Otherwise, the package might find the wrong \code{xmlparser.dll} which is included in both, \lib{GTK+} and Windows itself (and is significantly outdated in Windows).

If you are planning to compile \Rpackage{EBImage} from source, install \lib{GTK+} into a path that does not contain spaces. It is strongly recommended to use the same directory where you installed MSYS and MinGW, namely \code{c:\\mingw}. This directory is hard coded into \code{.\\src\\Makevars.win} and you will need to modify this file if installing \lib{GTK+} elsewhere.
