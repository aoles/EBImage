%\VignetteIndexEntry{Image analysis with EBImage}
%\VignetteDepends{}
...
\vspace*{.1in}
\begin{center}
\scalebox{0.3}{\includegraphics{umask.jpg}}
\end{center}
...
\vspace*{.1in}
\begin{center}
\scalebox{0.3}{\includegraphics{channels.jpg}}
\end{center}
...
\vspace*{.1in}
\begin{center}
\scalebox{0.3}{\includegraphics{analysis.jpg}}
\end{center}
...
+  e <- fdraw(x2, ref=rgb, fill=F)
+ ## FIXME: fdraw still to be ported from wsPaint
+  f <- fdraw(x2, ref=i3, opac=0.15)
+  f <- fdraw(x1, ref=f,  opac=0.15)
+ }
\end{verbatim}
%$
