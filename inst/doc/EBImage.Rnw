...
\author{Gregoire Pau, Oleg Sklyar, Wolfgang Huber\\\email{osklyar@ebi.ac.uk}}
\maketitle
...
or URLs. The supported image formats depend on the capabilities of
...
\code{reize}, \code{rotate}, \code{translate} and the functions 
...
of pixels from an image and relabel these sets with an unique increasing
...
\code{max(eilabel)}), they have to be divided by these number before 
...
their neighbors, where the neighborood is specified by a filter matrix.
